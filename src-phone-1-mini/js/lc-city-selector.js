...
class LcCity {
    ...
    renderNav() {
        var _line = () => {
            ...
            setTimeout(() => {
                addClass(aSpan[index - 1], 'active');   // 给对应的选项添加class

                // 如果是回填，这里添加class有点不一样了
                if (this.isFill) {
                    this.fillCount.nav = true;    // 只执行一次
                    for (let i = 0, length = aSpan.length; i < length; i++) {
                        removeClass(aSpan[i], 'active');
                    };
                    addClass(aSpan[aSpan.length - 1], 'active');   // 给对应的选项添加class
                };
            }, 30);
    ...
    errorTip(errMsg) {
        ...
        addClass(lcErrorTip, 'lc-show');
        setTimeout(() => {
            removeClass(lcErrorTip, 'lc-show');
        }, 1500);
    }
    ...
    addTranx() {
        ...
        if (!hasClass(lcCenter, 'tranx')) {
            addClass(lcCenter, 'tranx');
        }
    }
    // 取消容器的偏移
    removeTranx() {
    ...
    show(event) {
        // 判断是不是pc端打开
        if (getPageSize().windowWidth > 768 && event) {
        ...
        addClass(this.el.lcCitySelect, 'lc-show');
        // 回填
    ...
    close() {
        ...
        addClass(lcCitySelect, 'lc-close');

        setTimeout(() => {
            removeClass(lcCitySelect, 'lc-show');
            removeClass(lcCitySelect, 'lc-close');
        }, 300);
    }
...
class LcCityInline {
    ...
    renderHtml() {
        ...
        let div = document.getElementById(this.option.baseID);
        div.className = 'lc-city-select lc-city-select-inline';
        div.innerHTML = html;
    }
    // 获取页面元素
    getElement() {
        // 页面元素
        let lcCitySelect = document.getElementById(this.option.baseID);
        this.el = {
            lcCitySelect: lcCitySelect,
            lcErrorTip: lcCitySelect.getElementsByClassName('lc_error_tip')[0],
            lcCancel: lcCitySelect.getElementsByClassName('lc_cancel')[0],
